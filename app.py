...
def register():
    ...
    if request.method == 'POST':
        ...
        # 在注册表单提交时检查是否是老用户
        existing_participant = Participant.query.filter_by(nickname=nickname,contact_info=contact).first()
        if existing_participant:
        # 自动登录这位老用户
        session['participant_id'] = existing_participant.id
        # 根据投票状态跳转
        if existing_participant.has_voted:
            return redirect(url_for('results'))
        else:
            return redirect(url_for('vote'))
        # 创建新参与者
